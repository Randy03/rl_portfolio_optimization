...
class Critic(tf.keras.Model):
    def __init__(self,state_dim,action_dim):
        ...
        self.layer_4 = tf.keras.layers.Dense(action_dim-1, activation='relu')
        ...
        self.layer_10 = tf.keras.layers.Conv2D(filters=20, kernel_size=(1,10), activation='relu', kernel_regularizer=tf.keras.regularizers.l2(0.001))
        self.layer_10_ = tf.keras.layers.Conv2D(filters=60, kernel_size=(1,30), activation='relu', kernel_regularizer=tf.keras.regularizers.l2(0.001))
        self.layer_10__ = tf.keras.layers.Conv2D(filters=10, kernel_size=(1,10), activation='relu', kernel_regularizer=tf.keras.regularizers.l2(0.001))

        self.layer_11 = tf.keras.layers.Conv2D(filters=1, kernel_size=(1,1), kernel_regularizer=tf.keras.regularizers.l2(0.001))
        self.layer_12 = tf.keras.layers.Dense(action_dim-1, activation='relu')
    ...
    def call(self, obs,actions):
        ...
        x1 = self.layer_2(x1)
        x1 = self.layer_2_(x1)
        x1 = self.layer_2__(x1)
        x1 = tf.concat([x1,_weights],3)
        x1 = self.layer_3(x1)
        x1 = tf.concat([_bias,x1],1)
        x1 = tf.keras.layers.Flatten()(x1)
        x1 = tf.keras.layers.Activation(custom_activation)(x1)
        x1 = self.layer_4(x1)
        ...
        x2 = self.layer_10(x2)
        x2 = self.layer_10_(x2)
        x2 = self.layer_10__(x2)
        x2 = tf.concat([x2,_weights],3)
        x2 = self.layer_11(x2)
        x2 = tf.concat([_bias,x2],1)
        x2 = tf.keras.layers.Flatten()(x2)
        x2 = tf.keras.layers.Activation(custom_activation)(x2)
        x2 = self.layer_12(x2)
